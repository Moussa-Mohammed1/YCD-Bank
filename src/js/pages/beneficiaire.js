...
function creercarte(benefeciaire) {
         ...
         <button id="menu-btn-${benefeciaire.id}" class=" absolute top-2 right-2 text-gray-500">
             <i class="fa-solid fa-ellipsis-vertical"></i>
                                   ...
                                   <div id="menu-${benefeciaire.id}"  class="menu-options  absolute top-8 right-2 bg-white shadow-lg rounded-lg border" style="display: none;">
                                     <button onclick="bloquerDebloquer(${benefeciaire.id})" class="block w-full text-left px-4 py-2 hover:bg-gray-100">Bloquer/Debloquer</button>
                                     <button onclick="supprimerCarte(${benefeciaire.id})" class="block w-full text-left px-4 py-2 hover:bg-gray-100">Bloquer/supprimer</button>
                                   

           <div>
                    <p class="font-medium text-gray-800">${benefeciaire.nom}</p>
    ...
    const btn = document.getElementById(`menu-btn-${benefeciaire.id}`);
    const menu = document.getElementById(`menu-${benefeciaire.id}`);
...
function supprimerCarte(id) {
    ...
    if (confirmation) {
        ...
        alert("Bénéficiaire supprimé avec succés !");
    }
}